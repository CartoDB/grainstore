...
var MMLStore = function(redis_opts, optional_args) {  
  ...
  var redis_pool = redis_opts.pool ?
  ...
  ///
  me.purgeLocalizedResources = function(ttl, callback)
  {
    var now = Date.now();
...
//console.log("Scanning cache dir " + toclear);
      ...
      if ( err ) {
        ...
        var purgeNext = function() {
          ...
          fs.stat(file, function(err, stats) {
            if ( err ) {
              console.log("Cannot stat file " + file + ': ' + err);
              purgeNext();
            }
            if ( ttl ) {
              var cage = ( now - stats.ctime.getTime() ) / 1000;
              var aage = ( now - stats.atime.getTime() ) / 1000;
              //console.log("File " + file + " was created " + cage + " seconds ago and accessed " + aage + " seconds ago. Requested time to leave is " + ttl + " seconds");
  ...
  var gcruns = 0; // TODO: move in outer scope
    ...
    Step(
      ...
      function expireTokens(err, matches){
        ...
        var processNext = function() {
          ...
          {
              if ( err ) {
                console.log(id +": processing token " + token + ": " + err);
      ...
      function purgeLocalizedCache(err) {
        ...
        me.purgeLocalizedResources(ttl, this);
      },
      function finish(err) {
        if (redis_client)
