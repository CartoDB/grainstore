...
MMLBuilderInline.prototype.render = function(style_in, version, callback){
    const self = this;
    let style = '';
    let mml;
    ...
    try {
        if (style_in) {
            style = this.transformStyle(style_in, version);
        }
...
MMLBuilderInline.prototype.toMML = function(style_in){
...
function getLayerProperties(params, i) {
    ...
    ZOOM_PROPS.forEach(function(prop) {
        if (zoom.hasOwnProperty(prop) && Number.isFinite(zoom[prop])) {
            properties[prop] = zoom[prop];
        }
    });

    Object.keys(params).forEach(function(key, index) {
        if (SUPPORTED_PROPS.hasOwnProperty(key)) {
