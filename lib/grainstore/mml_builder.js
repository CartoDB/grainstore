...
var MMLBuilder = function(redis_pool, opts, optional_args, init_callback){
    ...
    if ( extra_config.mapnik_tile_format ) grainstore_map.format = extra_config.mapnik_tile_format;
        ...
        Step(
            ...
            function setStore(err){
                ...
                if ( store_key == base_store_key ) {
                  ...
                  if ( opts.datasource_extend ) tostore.datasource_extend = opts.datasource_extend;
                  tostore.style = style;
    ...
    me.baseMML = function(args){
        ...
        for (var i=0; i<tables.length; ++i) {
          ...
          if ( opts.gcols && opts.gcols[i] ) {
            if (_.isString(opts.gcols[i])) {
                datasource.geometry_field = opts.gcols[i];
            } else {
                var gcol = opts.gcols[i];
          ...
          if ( opts.datasource_extend && opts.datasource_extend[i] ) {
              datasource = _.extend(datasource, opts.datasource_extend[i]);
          }

          if ( opts.extra_ds_opts ) {
            datasource = _.defaults(datasource, opts.extra_ds_opts[i]);
          }
