    ...
    me.init = function(callback){
        ...
        Step(
            ...
            function setStore(err, compiled_XML){
                ...
                if ( ! opts.table ) {
                    redis_transaction.EXPIRE(store_key, ttl || default_ttl);
                }
    ...
    me.render = function(style, callback, version){
        ...
        if ( version != target_mapnik_version ) {
          try {
            ...
            if ( _.isArray(style) ) {
              var style_in = style; style = []
              for ( var i=0; i<style_in.length; ++i ) {
                style[i] = t.transform(style_in[i], version,
                                                    target_mapnik_version);
              }
            } else {
...
//console.log("Resolved mml: "); console.dir(mml);
    ...
    me.toMML = function(style_in){
        ...
        for (var i=0; i<style.length; ++i) {
          var stylesheet  = {};
          stylesheet.id   = _.isArray(style_in) ? ('style' + i) : 'style.mss';
    ...
    me.makeToken = function() {
      ...
      if ( style_override ) {
        ...
        md5.update(style_version_override); /* this cannot be an array (yet) */
      }
