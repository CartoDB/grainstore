...
// `layer`           - Interactivity layer, to use with token and grids
...
var MMLBuilder = function(redis_pool, opts, optional_args, init_callback){
    ...
    me.render = function(style_in, callback, version){
        ...
        millstone.resolve(millstone_options, function(err, mml) {
          ...
          // NOTE: we _might_ need a new object here because carto writes into it
          var carto_env = extra_config.carto_env || {};
    ...
    me.getStyle = function(callback, convert){
        ...
        Step(
            function initStyle(){
                that.init(this, true); // allow broken css
            },
            function getRedisClient(err, data){
                if (err) throw err;
                redis_pool.acquire(grainstore_styles.db, this);
            ...
            function releaseClient(err, data){
                if (!_.isUndefined(redis_client))
                    redis_pool.release(grainstore_styles.db, redis_client);
                if ( err ) throw err;
                if ( _.isNull(data) ) throw new Error("mml_builder's redis key disappeared");
    ...
    me.toXML = function(callback){
        this.init(function(err, data) {
            if (err) {
                ...
                var parsed = JSON.parse(data);
                if ( ! parsed.xml ) {
                  callback(new Error("Unable to render XML data"));
                  return;
                }
                if ( ! _.isArray(parsed.xml) ) {
                  callback(new Error("rendered XML data is not an array"));
                  return;
                }
                if ( ! parsed.xml[interactivity_layer] ) {
                  callback(new Error("mml_builder did not render XML data for interactivity layer " + interactivity_layer));
                  return;
                }
                callback(err, parsed.xml[interactivity_layer]);
            }
        }, false); // do not allow broken css
    };
