...
var MMLBuilder = function(redis_pool, opts, optional_args, init_callback){
    ...
    //
    // The function will not re-construct the XML if the ``force`` parameter
    // isn't true and the XML is already found in the redis-stored payload.
    //
    ...
    me.init = function(callback){
        ...
        Step(
            ...
            function renderBaseStyleOrDefaultOrOverride(err, data){
                if (err) throw err;
                if (_.isNull(data)){
                    style = that.processStyle(grainstore_styles[geom_type]);
                } else {
                    style = that.processStyle(JSON.parse(data).style);
                }
            ...
            function setStore(err, compiled_XML){
                if (err) throw err;
                xml = compiled_XML;
                var tostore = {xml: compiled_XML};
                if ( store_key == base_store_key ) {
                  tostore.style = style;
                }
                var payload = JSON.stringify(tostore);
                redis_client.SET(store_key, payload, this);
            },
            function setBaseStore(err, data){
                if (err) throw err;
                if ( store_key != base_store_key && _.isNull(style_override) ) {
                  var tostore = {style: style};
                  var payload = JSON.stringify(tostore);
    ...
    me.resetStyle = function(callback){
      var that = this;
      that.getStyle(function(err, style) {
        return that.setStyle(style.style, callback);
      });
    };
        ...
        Step(
            ...
            function storeStyleAndXML(err, data){
                ...
                if ( _.isNull(style_override) )  {
                  ...
                  extended_store_key = extended_store_key.substring(0, extended_store_key.lastIndexOf('|'))
                    + '|' + base64.encode(style);
    ...
    // Perform any version trasnformation required to
    // target current mapnik target
    me.processStyle = function(style){

      var missing_version = '2.0.2';
      var target_version = target_mapnik_version;
      var re_ver = RegExp('version: \'([^\']*)\'');
      var got_version = re_ver.exec(style);
      var version = got_version ? got_version[1] : missing_version;

      if ( version != target_version ) {

        // From 2.0.x to 2.1.x we double marker-width and marker-height
        if ( version == '2.0.2' && target_version == '2.1.0' ) {
      ...
      if ( got_version ) {
        if ( version != target_version ) {
          style = style.replace(re_ver, "version: '" + target_version + "'");
        }
      }
      else style = "version: '" + target_version + "'; " + style;
    ...
    var style_override = null;
    if ( opts.style ) {
      try {
        style_override = me.processStyle(opts.style);
      } catch ( err ) {
        init_callback(err, null);
        return;
      }
    }
    ...
    if (!_.isUndefined(opts.sql) || !_.isNull(style_override)) {
    ...
    if (!_.isNull(style_override)) {
        extended_store_key = extended_store_key + '|' + base64.encode(style_override);
    }
