...
var convert20To21 = require('./paths/from20To21');
var convert23To30 = require('./paths/from23To30');

function StyleTrans () {}

module.exports = StyleTrans;

var reMapnikGeometryType = new RegExp(/\bmapnik-geometry-type\b/g);
function convertMapnikGeometryType (style) {
    return style.replace(reMapnikGeometryType, '"mapnik::geometry_type"');
}
...
StyleTrans.prototype.setLayerName = function (css, layername) {
    var ret = css.replace(/#[^\s[{;:]+\s*([:\[{])/g, '#' + layername + ' $1'); // eslint-disable-line
...
StyleTrans.prototype.transform = function (style, from, to) {
    ...
    if (semver.satisfies(from, '<2.2.0')) {
        style = convertMapnikGeometryType(style);
    }
    ...
    while (from !== to) {
        ...
        if (Object.prototype.hasOwnProperty.call(tp, from)) {
